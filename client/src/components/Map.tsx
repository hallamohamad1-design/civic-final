...
function MapEvents({ onLocationSelect, onMapReady, onLocationFound }: MapEventsProps) {
  ...
  useEffect(() => {
    ...
    console.log('[Map] MapEvents - Map initialized');
    map.invalidateSize();
    ...
    if (!hasLocated.current) {
      ...
      console.log('[Map] Attempting to locate user');
  ...
  useMapEvents({
    click(e) {
      console.log('[Map] Map clicked at:', e.latlng);
      if (onLocationSelect) {
        onLocationSelect({ lat: e.latlng.lat, lng: e.latlng.lng });
      }
    },
    locationfound(e) {
      console.log('[Map] User location found:', e.latlng);
      if (onLocationFound) {
...
export function MapView({
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      console.log('[Map] Searching for:', searchQuery);
      ...
      if (results.length > 0 && mapInstance) {
        ...
        console.warn('[Map] No results found for:', searchQuery);
      }
    } catch (error) {
    ...
    <div className={cn("w-full h-full rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-xl relative group bg-slate-100 dark:bg-slate-800", className)}>
      ...
      <div className="absolute top-4 right-4 z-[1000] flex flex-col gap-2">
        <Button 
          ...
          className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-md shadow-lg hover:bg-slate-50 dark:hover:bg-slate-700 h-11 w-11 rounded-xl border-slate-200 dark:border-slate-600 group/locate"
          title="My Location"
        >
        ...
        <LayersControl position="bottomright">
          ...
          <LayersControl.BaseLayer name="Satellite View">
            <TileLayer
              ...
              eventHandlers={{
                tileerror: (error) => {
                  console.error('[Map] Satellite tile error:', error);
                },
                load: () => {
                  console.log('[Map] Satellite tiles loaded successfully');
                }
              }}
            />
          </LayersControl.BaseLayer>
          <LayersControl.BaseLayer name="OpenStreetMap">
            <TileLayer
              ...
              eventHandlers={{
                tileerror: (error) => {
                  console.error('[Map] OSM tile error:', error);
                }
              }}
            />
          </LayersControl.BaseLayer>
