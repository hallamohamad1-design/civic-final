...
import { MapPin, ThumbsUp, EyeOff } from "lucide-react";
    ...
    <Link href={`/issues/${issue.id}`}>
      <Card
        className={`cursor-pointer transition-all hover:shadow-md border-none ${
          isSelected ? "ring-2 ring-primary bg-primary/5 dark:bg-primary/10" : "bg-slate-50 dark:bg-slate-700"
        }`}
        onClick={onClick}
      >
        <CardContent className="p-4 flex gap-3">
          {issue.imageUrl && (
            <div className="w-20 h-20 rounded-lg overflow-hidden flex-shrink-0 bg-slate-200 dark:bg-slate-600">
              <img src={issue.imageUrl} className="w-full h-full object-cover" alt={issue.title} />
            </div>
          )}
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start mb-1">
              <h4 className="font-bold text-slate-900 dark:text-white line-clamp-1 text-sm">{issue.title}</h4>
              <div className="flex items-center gap-1">
                {issue.isAnonymous === 1 && (
                  <Badge className="bg-purple-100 text-purple-600 border-none text-[9px] px-1.5 py-0 flex items-center gap-0.5">
