...
import { AlertCircle, Users, AlertTriangle, EyeOff, BarChart3, TrendingUp, Shield, Loader2, CalendarDays, RefreshCw, Download, LogOut, Bell, Check, Trash, CheckCheck, Flag } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  // Moderation flag count — admin-only badge
  const { data: flagCountData } = trpc.moderation.getPendingFlagCount.useQuery(
    undefined,
    { enabled: isAdmin, refetchInterval: 30_000 }
  );
  const flagCount = flagCountData?.count ?? 0;
  ...
  // 4. Pending anonymous reports
  const {
    data: pendingAnonymous,
    isLoading: anonLoading,
    refetch: refetchAnonymous,
  } = trpc.admin.getPendingAnonymous.useQuery(undefined, {
    enabled: isAdmin,
    refetchInterval: 30_000,
    retry: 2,
  });
  ...
  const approveAnonMutation = trpc.admin.approveAnonymous.useMutation({
    onSuccess: () => {
      utils.admin.getPendingAnonymous.invalidate();
      utils.admin.getAllIssues.invalidate();
      toast.success("Anonymous report approved and published.");
    },
    onError: () => toast.error("Failed to approve report."),
  });

  const rejectAnonMutation = trpc.admin.rejectAnonymous.useMutation({
    onSuccess: () => {
      utils.admin.getPendingAnonymous.invalidate();
      toast.success("Anonymous report rejected and removed.");
    },
  ...
  // ─── Sync All Data ────────────────────────────────────────────────
  const isSyncing = statsLoading || isIssuesLoading || hiddenLoading || anonLoading;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="bg-slate-900 text-white shadow-md">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {/* Moderation Flag Button — shows flagged item count */}
            <Link href="/moderation">
              <Button variant="ghost" size="icon" className="relative text-slate-300 hover:text-white hover:bg-slate-800" title="Content Moderation">
      ...
      <div className="container mx-auto py-8 px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* Pending Anonymous Reports */}
          <Card className="shadow-sm border-purple-100">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <EyeOff className="w-5 h-5 text-purple-500" />
                Pending Anonymous Reports
                {pendingAnonymous && pendingAnonymous.length > 0 && (
                  <Badge className="bg-purple-100 text-purple-700 border-none ml-1">{pendingAnonymous.length}</Badge>
                )}
              </CardTitle>
              <CardDescription>Review and approve or reject anonymous reports before they go public</CardDescription>
            </CardHeader>
            <CardContent>
              {anonLoading ? (
                <div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin" /></div>
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {pendingAnonymous.map((issue: any) => (
                    <div key={issue.id} className="p-4 border border-purple-100 rounded-xl bg-purple-50/40">
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <Badge className="bg-purple-100 text-purple-700 border-none text-[10px]">ANONYMOUS</Badge>
                          ...
                          <div className="mt-2 pt-2 border-t border-purple-100">
                            <p className="text-[10px] text-purple-600 font-medium">Reporter (visible to admin only):</p>
                            <p className="text-xs text-slate-600">{issue.userName || "Unknown"} — {issue.userEmail || "No email"}</p>
                          </div>
                        </div>
                      </div>
                      <div className="flex gap-2 mt-3">
                        <Button
                          ...
                          onClick={() => approveAnonMutation.mutate(issue.id)}
                          disabled={approveAnonMutation.isPending}
                        >
                          ✓ Approve & Publish
                        </Button>
                        <Button
                          ...
                          onClick={() => rejectAnonMutation.mutate(issue.id)}
                          disabled={rejectAnonMutation.isPending}
                        >
                          ✕ Reject
                        </Button>
                      </div>
                ...
                <p className="text-center text-slate-400 py-8 text-sm">No pending anonymous reports</p>
              )}
            </CardContent>
          </Card>
                ...
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                  {issues.map((issue: any) => (
                    <div key={issue.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-semibold text-primary">{issue.isAnonymous ? "🔒 Anonymous Reporter" : (issue.userName || "Unknown")}</span>
                        ...
                        {issue.userEmail && !issue.isAnonymous && (
                          <p className="text-xs text-muted-foreground mt-1">📧 {issue.userEmail}</p>
                        )}
                        {issue.isAnonymous === 1 && (
                          <p className="text-xs text-purple-500 mt-1">📧 {issue.userEmail} (admin only)</p>
                        )}
                      </div>
