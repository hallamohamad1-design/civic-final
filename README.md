...
|----------|-------------|---------|
| `VITE_GOOGLE_MAPS_API_KEY` | Google Maps JavaScript API key | (not set) |
...
3. **Create API Key**:
   ...
   - Optionally restrict the key by domain for security
...
**Note**: Without a Google Maps API key, the application will show a friendly fallback message with setup instructions. The app will still function with basic location features using OpenStreetMap as a fallback for geocoding.
